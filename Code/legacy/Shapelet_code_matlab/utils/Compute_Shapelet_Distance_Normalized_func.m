function [ distance, distance_location_offset ] = Compute_Shapelet_Distance_Normalized_func(BASE_TIME_SERIES, SHAPELET)
...
normalized_shapelet = (SHAPELET - mean(SHAPELET))/std(SHAPELET);
...
while (end_index <= time_series_length)
    subseries = BASE_TIME_SERIES(1, start_index:end_index);
    normalized_subseries = (subseries - mean(subseries))/std(subseries);
    same_length_subtimeseries(start_index,:) = normalized_subseries;
...
shapelet_repmat = repmat(normalized_shapelet, time_series_length - shapelet_length + 1, 1);
