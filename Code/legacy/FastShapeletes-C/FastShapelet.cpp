...
void error(int type) {
  ...
  case 0:
    ...
    printf(" == Syntax ==\n");
    printf(" shapelet.exe  Train_Data   Number_of_Class  Number_of_Instance  "
           "Max_Length  Min_Length Step [R] [distance] [cid] [normalize] "
           "[top_k] [output_tree_file] "
           "[output_time_file]\n\n");
    printf(" == Examples ==\n");
    printf(" shapelet.exe  ECG200_TRAIN  2  100  96  1  1  \n");
    printf(" shapelet.exe  ECG200_TRAIN  2  100  96  1  1 10 "
           " \n");
    printf(" shapelet.exe  ECG200_TRAIN  2  10   30  20 1 10  \"euclidean\" 0 "
           "1 1 tree_ECG.txt  "
           "time_ECG.txt \n");
    printf("\n");
    break;
...
/// Fix card = 4 here !!!
SAX_word_type CreateSAXWord(const double *sum_segment, const int *elm_segment,
                            double mean, double std, int sax_len) {
...
double CalInfoGain2(vector<int> const &c_in, vector<int> const &c_out,
                    int const &total_c_in, int const &total_c_out) {
  return class_entropy -
         ((double)(total_c_in) / num_obj * EntropyArray(c_in, total_c_in) +
          (double)(total_c_out) / num_obj * EntropyArray(c_out, total_c_out));
}

/// From top-k-score SAX
/// Calculate Real Infomation Gain
Shapelet FindBestSAX(int top_k, distfunc dist_func, bool cid, bool norm) {
  ...
  for (int k = 0; k < min(top_k, (int)Score_List.size()); k++) {
    ...
    for (kk = 0; kk < min((int)usax.sax_id.size(), 1); kk++) {
      ...
      for (int obj = 0; obj < num_obj; obj++) {
        dist = NearestNeighborSearch(query, Data[obj], obj, Q, dist_func, cid,
                                     norm);
...
void SetCurData(int node_id) {
  if (node_id == 1) {
    ...
    for (it = Node_Obj_List[node_id].begin();
         it != Node_Obj_List[node_id].end(); it++) {
...
void SetNextNodeObj(unsigned int node_id, Shapelet sh, distfunc dist_func,
                    bool cid, bool norm) {
  int q_obj = sh.obj;
  int q_pos = sh.pos;
  int q_len = sh.len;
  ...
  for (int obj = 0; obj < num_obj; obj++) {
    dist =
        NearestNeighborSearch(query, Data[obj], obj, Q, dist_func, cid, norm);
    if (dist <= dist_th) {
      left_num_obj++;
  ...
  if ((sum_c_in <= MIN_OBJ_SPLIT) ||
      ((double)max_c_in / sum_c_in >= MAX_PURITY_SPLIT))
    left_is_leaf = true;
  if ((sum_c_out <= MIN_OBJ_SPLIT) ||
      ((double)max_c_out / sum_c_out >= MAX_PURITY_SPLIT))
    right_is_leaf = true;
...
void printTree(FILE *fo, int argc, char **argv, int seed) {
  ...
  for (unsigned int i = 1; i < Node_Obj_List.size(); i++) {
    ...
    {
      fprintf(fo, "NonL %2d    %2s    %3d  %3d  %3d  %7.4f ", i, "--", sh.obj,
              sh.pos, sh.len, sh.dist_th);

      fprintf(fo, "==> ");
      for (int j = 0; j < num_class; j++)
        fprintf(fo, "%3d ", sh.c_in[j]);
      fprintf(fo, " / ");
      for (int j = 0; j < num_class; j++)
        fprintf(fo, "%3d ", sh.c_out[j]);
      fprintf(fo, "\n");
...
int main(int argc, char *argv[]) {
  ...
  char dist_func_name[100] = "euclidean";
  double percent_mask;
  Shapelet sh;
  distfunc dist_func;
  ...
  if (argc > 8)
    strcpy(dist_func_name, argv[8]);

  dist_func = get_distance_function(dist_func_name);
  ...
  char result_file_name[200] = "tree.txt";
  if (argc > 12)
    strcpy(result_file_name, argv[12]);

  char time_file_name[200] = "time.txt";
  if (argc > 13)
    strcpy(time_file_name, argv[13]);
  ...
  printf("num_class=%d, num_obj=%d, R=%d, top_k=%d, percent_mask=%.2f, "
         "tree_depth=%d    tree_file=%s  time_file=%s distance=%s CID=%s "
         "normalize=%s\n\n",
         num_class, num_obj, R, top_k, percent_mask,
         int(pow(2, max_tree_depth)), result_file_name, time_file_name,
         dist_func_name, (cid ? "true" : "false"), (norm ? "true" : "false"));

  ReadTrainData(train_file_name);

  /// Find Shapelet
  for (node_id = 1; (node_id == 1) || (node_id < (int)Node_Obj_List.size());
       node_id++) {
    Shapelet bsf_sh = Shapelet();
    if (node_id <= 1)
      SetCurData(node_id);
    else if (Classify_list[node_id] ==
             -1) /// non-leaf node (-1:body node, -2:unused node)
      SetCurData(node_id);
    else
      continue;
    ...
    for (subseq_len = min_len; subseq_len <= max_len; subseq_len += step) {
      ...
      sh = FindBestSAX(top_k, dist_func, cid, norm);
      t5 = clock();
      s4 += t5 - t4;
    ...
    if (bsf_sh.len > 0) {
      ...
      SetNextNodeObj(node_id, bsf_sh, dist_func, cid, norm);
    }
  }
  t6 = clock();
