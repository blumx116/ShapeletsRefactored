...
void error(int type) {
  ...
  case 0:
    ...
    printf(" == Syntax ==\n");
    printf(
        " Classify.exe  Test_Data  Number_of_Class  Number_of_Instance  "
        "[distance] [cid] [normalize] [input_tree_file] [output_acc_file]\n\n");
    printf(" == Examples ==\n");
    printf(" Classify.exe  ECG200_TRAIN   2  100  \n");
    printf(" Classify.exe  CBF_TEST       3  900 \"euclidean\" 0 1 tree_CBF\n");
    printf(" Classify.exe  GUN_POINT_TEST 2  150  \"euclidean\" 0 1 tree_GUN  "
           "acc_GUN\n");
    printf("\n");
    break;
  case 1:
    ...
    break;
  case 2:
    printf("Error!! Number of Classes is more than input parameters. Please "
           "check.\n");
    break;
  }
  exit(0);
...
void ReadData(char *filename) {
  ...
  for (unsigned int i = 0; i < Label.size(); i++) {
    ...
    if (it == Local_Label.end()) { /// New label
...
void ReadShapeletTree(char *result_file_name) {
  ...
  //    fprintf(fo,"Node %2d    %2s    %3d  %3d  %3d  %7.4f
  //    ",i,"--",sh.obj,sh.pos,sh.len,sh.dist_th);
  /// Note that Each line must have the same number of objects
...
void Classification(int &count_correct, int &count_wrong, CFMatrix_Type &Matrix,
  ...
  for (unsigned int obj = 0; obj < Data.size(); obj++) {
    ...
    while ((Leaf_list[node_id] == false) || (node_id > max_node_id)) {
      ...
      d = NearestNeighborSearch(Final_Sh[node_id].ts, Data[obj], 0, Q,
                                dist_func, cid, norm);
...
int main(int argc, char *argv[]) {
  ...
  char tree_file_name[200] = "tree.txt";
  if (argc > 7)
    strcpy(tree_file_name, argv[7]);

  char time_file_name[200] = "time.txt";
  if (argc > 8)
    strcpy(time_file_name, argv[8]);
  ...
  printf("num_class=%d, num_obj=%d  distance=%s CID=%s normalize=%s %s  %s\n\n", num_class, num_obj,
         dist_func_name, (cid ? "true" : "false"), (norm ? "true" : "false"),
         tree_file_name, time_file_name);
  ...
  Classification(count_correct, count_wrong, Matrix, dist_func, cid, norm);
  ...
  fprintf(f_time, "; accuracy= %.3f   Correct:%d , Wrong: %d\n\n",
          (double)count_correct / (count_correct + count_wrong) * 100,
          count_correct, count_wrong);
